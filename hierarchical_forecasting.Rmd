...
The data is obtained from the StatsWales website and it is publicly accessible.

Data source: https://statswales.gov.wales/Catalogue/Health-and-Social-Care/NHS-Hospital-Waiting-Times/emergency-department/emergencyattendances-by-age-sex-site

The data will be split up into the subsequent sections:

I'll use the following forecasting techniques:

- Base forecasting: I use base forecasting mainly for the attendance under the different local health boards, in order to conduct the exploratory analysis and also to determine which model is a better fit for the data.
...
https://github.com/JaniceHsinHSU/NHS_forecast
...
**Explanation for the change of local health boards (LHB)**
...
Healthcare systems around the world faced significant difficulties during the COVID-19 pandemic, which caused them to quickly shift into crisis-response mode. Intriguing trends during and after the epidemic era have been revealed by a thorough review of attendance data from local health boards.
...
The number of months in the training data is:
```{r}
library(tsibble)
